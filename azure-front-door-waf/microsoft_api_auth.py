        ...
        if CONFIG_SUPPORTS_TOKEN:
            if not 'accessToken' in config:
                ...
                update_connnector_config(connector_info['connector_name'], connector_info['connector_version'], config,
                                         config['config_id'])
                endpoint = f'{ms.host}/subscriptions/{ms.subscription_id}/resourceGroups/{ms.resource_group_name}/providers/Microsoft.Network/azureFirewalls?api-version={ms.api_version}'
                headers = {'Authorization': "Bearer {0}".format(config.get('accessToken')), 'Content-Type': 'application/json'}
                resp = request("GET", endpoint, headers=headers, verify=ms.verify_ssl)
                if resp.status_code == 200:
                    ...
                    if resp.text != "":
                        ...
                        error_msg = '{0}: {1}'.format(resp.status_code, resp.reason)
                        raise ConnectorError(error_msg)
            else:
                token_resp = ms.validate_token(config, connector_info)
                endpoint = f'{ms.host}/subscriptions/{ms.subscription_id}/resourceGroups/{ms.resource_group_name}/providers/Microsoft.Network/azureFirewalls?api-version={ms.api_version}'
                headers = {'Authorization': token_resp,
